...
function extractMetadata(content, filePath, profile) {
  ...
  if (frontmatterMatch) {
    ...
    // Priority: source first, then url (for clippings)
    const sourceMatch = frontmatter.match(/source:\s*(.+)/i);
    if (sourceMatch) {
      metadata.source = sourceMatch[1].trim().replace(/["']/g, '');
    } else {
      // Fallback to url property if source doesn't exist
      const urlMatch = frontmatter.match(/url:\s*(.+)/i);
      if (urlMatch) {
        metadata.source = urlMatch[1].trim().replace(/["']/g, '');
      }
    }
